...
async def layered(ctx, *, message: str):
    args = message.split()
    if len(args) != 6:
        await ctx.send("Expecting a total of 6 parameters, please specify them, see /help")
        return
    ...
    range1 = args[4]
    range2 = args[5]
    await ctx.send(re.sub(r'<.*?>', '', range_calc.layered(range_strategy, size_strategy, size_param, size_amount, range1, range2)))
